...
impl Indexer {
    ...
    pub fn search(
        ...
        let defaule_fileds: Vec<_> = fields.iter().map(|s| s.as_str()).collect();
        let data: Vec<_> = self
            .0
            .search(&query, &defaule_fileds, limit, offset.unwrap_or(0))
            .map_err(to_pyerr)?
            .into_iter()
            .map(|(source, doc)| (source, serde_json::to_string(&doc).unwrap()))
            .collect();
        Ok(data)
    }
        ...
        let conversion = conversion
            ...
            .filter_map(|(k, (t1, t2))| {
                let t = match (t1.as_ref(), t2.as_ref()) {
                    ...
                    ("number", "number") => (x::ValueType::Number, x::ValueType::Number),
                    _ => return None,
                };
                Some(((k, t)))
            })
