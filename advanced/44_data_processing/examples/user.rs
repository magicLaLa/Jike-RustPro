...
/// 使用 FromRow 派生宏从数据库中读取出来的数据专为成 User 结构
#[allow(dead_code)]
...
impl UserDb {
    ...
    /// 用户注册: 在 users 表中储存 argon2 哈希过的密码
    ...
    pub async fn login(&self, email: &str, password: &str) -> Result<String> {
      ...
      if let Err(_) = verify_password(password, &user.hashed_password) {
          return Err(anyhow!("failed to length"));
      }
...
async fn main() -> Result<()> {
  let url = env::var("DATABASE_URL").unwrap_or("sqlite://advanced/44_data_processing/data/example.db".into());
  // 创建链接池
