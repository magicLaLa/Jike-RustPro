...
use axum::{AddExtensionLayer, Router, extract::{Path, Extension}, handler::get, http::{HeaderMap, HeaderValue, StatusCode}};
use bytes::Bytes;
use lru::LruCache;
...
async fn main() {
    ...
    let app = Router::new()
        .route("/image/:spec/:url", get(generatr))
        .layer(
            ServiceBuilder::new()
...
async fn retrieve_image(url: &str, cache: Cache) -> Result<Bytes> {
    ...
    let g = &mut &mut cache.lock().await;
